...
CzSIQSWallet::CzSIQSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zSIQS seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSIQSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSIQSSeed_deprecated()) {
                    LogPrintf("%s: Updated zSIQS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zSIQS seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zSIQS wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSIQSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSIQSCount(nCountLastUsed);
    else if (!walletdb.ReadZSIQSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSIQSWallet::Lock()
{
    seedMaster = 0;
}

void CzSIQSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSIQSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSIQS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSIQSWallet::LoadMintPoolFromDB()
{
...
void CzSIQSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSIQSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSIQSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zSIQSTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSIQSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSIQS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zSIQSTracker which also adds to database
    pwalletMain->zSIQSTracker->Add(dMint, true);
...
void CzSIQSWallet::SeedToZSIQS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSIQSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSIQSWallet::GenerateDeterministicZSIQS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSIQSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSIQSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
