...
#ifndef SIQS_ZSIQSWALLET_H
#define SIQS_ZSIQSWALLET_H
...
public:
    CzSIQSWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZSIQS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZSIQS(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //SIQS_ZSIQSWALLET_H
