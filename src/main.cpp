...
#if defined(NDEBUG)
#error "SIQS cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uSIQS) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
   ...
   if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
       return error("%s : zSIQS spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zSIQS is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zSIQS spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zSIQS without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zSIQS spend with serial %s from tx %s is not in valid range\n", __func__,
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    if (GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE) && tx.ContainsZerocoins())
        return state.DoS(10, error("AcceptToMemoryPool : Zerocoin transactions are not supported by SIQS"), REJECT_INVALID, "bad-tx");
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zSIQS spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zSIQS");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zSIQS mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 1) {
    ...
    } else if(nHeight >= 2 && nHeight < 200) {
        nSubsidy = 1 * COIN;
    } else if(nHeight >= 200 && nHeight < 6000) {
        nSubsidy = 35 * COIN;
    } else if(nHeight >= 6000 && nHeight < 14000) {
        nSubsidy = 20 * COIN;
    } else if(nHeight >= 14000 && nHeight < 35000) {
        nSubsidy = 16 * COIN;
        ...
        int nMul = (nHeight-700000)/350000;
        if(nMul < 32)
			nSubsidy = (7.5 / pow(2,nMul)) * COIN;
		else
			nSubsidy = 0; //down to zero
    } else {
        nSubsidy = 2 * COIN;  //everything else
    }
    return nSubsidy;
}


int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZSIQSStake)
{
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from SIQS
...
{
    RenameThread("SIQS-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZSIQSMinted()
{
...
void RecalculateZSIQSSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zSIQS supply
...
bool RecalculateSIQSSupply(int nHeightStart)
{
...
{
    // SIQS: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZSIQSSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (block.nTime > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE) && !IsInitialBlockDownload() && tx.ContainsZerocoins()) {
            return state.DoS(100, error("ConnectBlock() : zerocoin transactions are not supported by SIQS"));
        }
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculateZSIQSMinted();
        RecalculateZSIQSSpent();
        RecalculateSIQSSupply(Params().Zerocoin_StartHeight());
    }

    //Track zSIQS money supply in the block index
    if (!UpdateZSIQSSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zSIQS supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zSIQSSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert SIQS to zSIQS
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zSIQS spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zSIQS serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zSIQS stake block is less than allowed start height", __func__);

    if (CZSIQSStake* zSIQS = dynamic_cast<CZSIQSStake*>(stake)) {
        CBlockIndex* pindexFrom = zSIQS->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zSIQS stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
             return error("%s: zSIQS stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zSIQS->GetValue()));
        if (nChecksum200 != zSIQS->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zSIQS->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZSIQS() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zSIQS fails context checks", __func__));
    ...
    if (isPoS) {
       ...
       // Inputs
       std::vector<CTxIn> SIQSInputs;
       std::vector<CTxIn> zSIQSInputs;

       for (const CTxIn& stakeIn : stakeTxIn.vin) {
           if(stakeIn.scriptSig.IsZerocoinSpend()){
               zSIQSInputs.push_back(stakeIn);
           }else{
               SIQSInputs.push_back(stakeIn);
           }
       }
       const bool hasSIQSInputs = !SIQSInputs.empty();
       const bool hasZSIQSInputs = !zSIQSInputs.empty();
       ...
       for (const CTransaction& tx : block.vtx) {
           for (const CTxIn& in: tx.vin) {
               ...
               if(hasSIQSInputs)
                   ...
                   for (const CTxIn& SIQSIn : SIQSInputs){
                       if(SIQSIn.prevout == in.prevout){
       ...
       if (isBlockFromFork) {
           ...
           do {
               ...
               for (const CTransaction& t : bl.vtx) {
                   for (const CTxIn& in: t.vin) {
                       ...
                       for (const CTxIn& stakeIn : SIQSInputs) {
           ...
           // Now that this loop if completed. Check if we have zSIQS inputs.
           if(hasZSIQSInputs){
               for (const CTxIn& zPivInput : zSIQSInputs) {
                   ...
                   if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
                       return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                  stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zSIQS");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zSIQS mints and %d zSIQS spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // SIQS: We use certain sporks during IBD, so check to see if they are
