...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered SIQS address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 SIQS.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal SIQS:URI (SIQS:XyZ...?message=example)
            {
...
static void NotifyzSIQSReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzSIQSReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzSIQSReset.connect(boost::bind(NotifyzSIQSReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzSIQSReset.disconnect(boost::bind(NotifyzSIQSReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zSIQSTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
