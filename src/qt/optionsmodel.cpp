...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::SIQS);
    ...
    if (!settings.contains("nAnonymizeSIQSAmount"))
        settings.setValue("nAnonymizeSIQSAmount", 1000);

    nAnonymizeSIQSAmount = settings.value("nAnonymizeSIQSAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeSIQSAmount"))
        SoftSetArg("-anonymizeSIQSamount", settings.value("nAnonymizeSIQSAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in SIQS.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSIQSAmount:
            return QVariant(nAnonymizeSIQSAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeSIQSAmount:
            nAnonymizeSIQSAmount = value.toInt();
            settings.setValue("nAnonymizeSIQSAmount", nAnonymizeSIQSAmount);
            emit anonymizeSIQSAmountChanged(nAnonymizeSIQSAmount);
            break;
        case CoinControlFeatures:
