...
#include "ui_zSIQScontroldialog.h"
...
std::set<std::string> ZSIQSControlDialog::setSelectedMints;
std::set<CMintMeta> ZSIQSControlDialog::setMints;

bool CZSIQSControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZSIQSControlDialog::COLUMN_DENOMINATION || column == ZSIQSControlDialog::COLUMN_VERSION || column == ZSIQSControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZSIQSControlDialog::ZSIQSControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZSIQSControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZSIQSControlDialog::~ZSIQSControlDialog()
{
    delete ui;
}

void ZSIQSControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSIQSControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZSIQSControlWidgetItem* itemDenom(new CZSIQSControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZSIQSControlWidgetItem *itemMint = new CZSIQSControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
...
void ZSIQSControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSIQSControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZSIQS_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSIQSControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZSIQSControlDialog::GetSelectedMints()
{
...
void ZSIQSControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
