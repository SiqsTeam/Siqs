...
public:
    ...
    bool WriteZSIQSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSIQSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSIQSSeed_deprecated(uint256& seed);
    bool EraseZSIQSSeed();
    bool EraseZSIQSSeed_deprecated();

    bool WriteZSIQSCount(const uint32_t& nCount);
    bool ReadZSIQSCount(uint32_t& nCount);
